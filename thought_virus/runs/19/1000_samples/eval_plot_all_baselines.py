...
for folder_name in os.listdir(ROOT_DIR):
    ...
    for subfolder in os.listdir(folder_path):
        ...
        if subfolder in BASELINE_IDS:
            baseline_results[subfolder] = np.array([agent0_mean, agent1_mean, agent2_mean])
        else:
    ...
    # Non-baseline subfolders
    for subfolder, values in baseline_results.items():
        plt.plot(
            x,
            values,
            marker="o",
    ...
    output_path = os.path.join(
        OUTPUT_DIR, f"{folder_name}_agent_comparison_baselines_plotted.png"
    )
