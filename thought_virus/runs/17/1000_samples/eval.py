...
def compute_column_means(csv_path, folder_name):
    ...
    return df[agent0_col].mean(), df[agent1_col].mean()
...
for folder_name in os.listdir(ROOT_DIR):
    ...
    for subfolder in os.listdir(folder_path):
        ...
        agent0_mean, agent1_mean = compute_column_means(csv_path, folder_name)

        if subfolder in BASELINE_IDS:
            baseline_values.append([agent0_mean, agent1_mean])
        else:
            non_baseline_results[subfolder] = [agent0_mean, agent1_mean]
    ...
    baseline_df = pd.DataFrame(
        baseline_values, columns=["agent0", "agent1"]
    )
    baseline_means = baseline_df.mean().values

    # ---- PLOTTING ----
    x = [0, 1]
    x_labels = ["agent0", "agent1"]
    ...
    output_path = os.path.join(
        OUTPUT_DIR, f"{folder_name}_agent_comparison.png"
    )
