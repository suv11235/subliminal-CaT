...
for animal in sorted(all_animals):
    ...
    for method in methods:
        ...
        # Find the best number folder (highest mean across all three agents)
        best_number = None
        best_mean = -np.inf
        ...
        print(f"  Method '{method}': best number folder = {best_number} (mean = {best_mean:.4f})")
    ...
    plt.title(f"Animal: {animal} - Best Results by Method")
    _, top = plt.ylim()
    plt.ylim(0, top)
    plt.legend()
    ...
    output_path = os.path.join(OUTPUT_DIR, f"{animal}_best_method_comparison.png")
